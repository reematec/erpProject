...
class ReemaBladderWindingIssue(models.Model):
    _name = 'reema.bladder.winding.issue'
    _description = 'Bladder Winding Issue'
    ...
    vendor_id = fields.Many2one(
        'res.partner', string='Winding Vendor', required=True,
    ...
    account_id = fields.Many2one(
        ...
        help='The raw bladder product being sent for winding, e.g. '
             '"SR Bladder, 190-200g, 196mm".',
        tracking=True,
    )
    qty = fields.Integer(string='Qty Sent', required=True, tracking=True)
    winding_weight = fields.Float(
        string='Winding Weight (g)', required=True, digits=(10, 2),
        help='Target thread weight the vendor is asked to wind onto each '
             'bladder. Drives the rate — winding is never exact, so the '
             'returned bladder weight is always a range around bladder '
             'weight + this figure.',
    )
    ...
    move_id = fields.Many2one(
        ...
        help='The move that took this batch of raw bladders from the MO\'s '
             'source location to the Bladder Winding (vendor) location.',
    )

    receipt_ids = fields.One2many('reema.bladder.winding.receipt', 'issue_id', string='Receipts')
    ...
    qty_wound = fields.Integer(
        compute='_compute_vendor_ledger', string='Wound',
    )
    ...
    def _compute_vendor_ledger(self):
        for rec in self:
            if not (rec.vendor_id and rec.account_id):
                rec.qty_issued_vendor = rec.qty_wound = rec.qty_accounted = rec.qty_outstanding = 0
                continue
            ...
            rec.qty_wound = sum(received.mapped('qty_wound'))
            rec.qty_accounted = rec.qty_wound + sum(received.mapped('qty_damaged')) + sum(received.mapped('qty_lost'))
    ...
    @api.constrains('qty', 'winding_weight')
    def _check_positive(self):
        for rec in self:
            if rec.qty <= 0:
                raise ValidationError(_('Qty Sent must be greater than 0.'))
            if rec.winding_weight <= 0:
                raise ValidationError(_('Winding Weight must be greater than 0.'))

    @api.model_create_multi
    def create(self, vals_list):
        for vals in vals_list:
            if vals.get('name', _('New')) == _('New'):
                vals['name'] = self.env['ir.sequence'].next_by_code('reema.bladder.winding.issue') or _('New')
    ...
    def action_print(self):
        return {
            ...
            'url': '/report/html/reema_inventory.report_bladder_winding_issue_slip/%s' % ','.join(str(i) for i in self.ids),
            'target': 'new',
        }
    ...
    def action_view_receipts(self):
        return {
            ...
            'name': _('Receipts'),
            'res_model': 'reema.bladder.winding.receipt',
            'view_mode': 'list,form',
    ...
    def action_confirm(self):
        vendor_loc = self.env.ref('reema_inventory.stock_location_bladder_winding')
        for rec in self:
            if rec.state != 'draft':
            ...
            move = self.env['stock.move'].create({
                'name': f'Bladder Winding Issue: {rec.account_id.display_name}',
                'product_id': rec.account_id.id,
...
class ReemaBladderWindingReceipt(models.Model):
    _name = 'reema.bladder.winding.receipt'
    _description = 'Bladder Winding Receipt'
    _inherit = ['mail.thread', 'mail.activity.mixin']
    _order = 'date desc, name desc'
    ...
    issue_id = fields.Many2one(
        'reema.bladder.winding.issue', string='Issue', required=True,
        domain=[('state', '=', 'dispatched')],
        tracking=True,
    )
    vendor_id = fields.Many2one(related='issue_id.vendor_id', string='Winding Vendor', store=True, readonly=True)
    ...
    winding_weight = fields.Float(related='issue_id.winding_weight', string='Winding Weight (g)', store=True, readonly=True)
    rate = fields.Float(related='issue_id.rate', string='Rate per Bladder (PKR)', store=True, readonly=True)

    wound_account_id = fields.Many2one(
        'product.product', string='Wound Bladder Account',
        help='The processed bladder product this batch is received into stock as, '
             'e.g. "SR Bladder, 190-200g, 196mm, Winding 60g".',
        tracking=True,
    )
    qty_wound = fields.Integer(string='Qty Wound (Payable)', default=0, tracking=True)
    qty_damaged = fields.Integer(string='Qty Damaged (Returned Unwound)', default=0, tracking=True)
    ...
    amount = fields.Float(
        ...
        help='qty_wound x rate — the vendor is paid for balls actually wound, not qty sent.',
    )
    state = fields.Selection(
    ...
    consume_move_id = fields.Many2one(
        ...
        help='Raw bladder (qty_wound) consumed from the vendor location.',
    )
    produce_move_id = fields.Many2one(
        'stock.move', string='Produce Move', readonly=True, copy=False,
        help='Wound bladder (qty_wound) produced into the MO source location.',
    )
    scrap_move_id = fields.Many2one(
        ...
        help='Raw bladder (qty_damaged + qty_lost) written off from the vendor '
             'location to Scrap — damaged bladders returned unwound, and '
             'bladders the vendor never returned.',
    )

    @api.depends('qty_wound', 'rate')
    def _compute_amount(self):
        for rec in self:
            rec.amount = rec.qty_wound * rec.rate

    @api.model_create_multi
    def create(self, vals_list):
        for vals in vals_list:
            if vals.get('name', _('New')) == _('New'):
                vals['name'] = self.env['ir.sequence'].next_by_code('reema.bladder.winding.receipt') or _('New')
        return super().create(vals_list)
    ...
    def action_print(self):
        return {
            'type': 'ir.actions.act_url',
            'url': '/report/html/reema_inventory.report_bladder_winding_receipt_slip/%s' % ','.join(str(i) for i in self.ids),
            'target': 'new',
        }
    ...
    def action_receive(self):
        ...
        for rec in self:
            ...
            if not rec.wound_account_id and rec.qty_wound:
                raise UserError(_('Select the Wound Bladder Account before confirming a receipt with wound quantity.'))
            total_this = rec.qty_wound + rec.qty_damaged + rec.qty_lost
            if total_this <= 0:
                raise UserError(_('Enter at least one quantity (Wound, Damaged, or Lost) before confirming.'))
            sibling_total = sum(
                (r.qty_wound + r.qty_damaged + r.qty_lost)
            ...
            if rec.qty_wound:
                ...
                rec.consume_move_id = rec._create_move(
                    rec.account_id, rec.qty_wound, vendor_loc, production_loc,
                    f'Bladder Winding Receipt: consume {rec.account_id.display_name}',
                ).id
                rec.produce_move_id = rec._create_move(
                    rec.wound_account_id, rec.qty_wound, production_loc, rec.issue_id.mo_id.location_src_id,
                    f'Bladder Winding Receipt: produce {rec.wound_account_id.display_name}',
                ).id
            ...
            if damaged_and_lost:
                ...
                rec.scrap_move_id = rec._create_move(
                    ...
                    f'Bladder Winding Receipt: scrap {rec.account_id.display_name}',
                ).id
            ...
            rec._message_log(
                body=_(
                    'Received: %(wound)s wound, %(damaged)s damaged, %(lost)s lost. '
                    'Amount due: PKR %(amount)s'
                ) % {
                    'wound': rec.qty_wound, 'damaged': rec.qty_damaged, 'lost': rec.qty_lost,
                    'amount': f'{rec.amount:,.2f}',
                }
            )


class ReemaBladderWindingBalance(models.Model):
    """Issued vs. Wound/Damaged/Lost ledger, one row per (MO, Vendor) —
    recomputed on every read so 'Outstanding' always reflects reality, same
    approach as reema.ilo.balance."""
    _name = 'reema.bladder.winding.balance'
    _description = 'Bladder Winding Issued vs Received Balance'
    _auto = False
    _order = 'vendor_id, mo_id'

    mo_id = fields.Many2one('mrp.production', string='Manufacturing Order', readonly=True)
    vendor_id = fields.Many2one('res.partner', string='Winding Vendor', readonly=True)
    qty_issued = fields.Integer(string='Qty Issued', readonly=True)
    qty_wound = fields.Integer(string='Qty Wound', readonly=True)
            ...
            CREATE OR REPLACE VIEW reema_bladder_winding_balance AS (
                ...
                LEFT JOIN (
                    SELECT i.mo_id, i.vendor_id,
                           SUM(r.qty_wound)   AS qty_wound,
                           SUM(r.qty_damaged) AS qty_damaged,
                           SUM(r.qty_lost)    AS qty_lost
                    FROM reema_bladder_winding_receipt r
                    JOIN reema_bladder_winding_issue i ON i.id = r.issue_id
    ...
    def action_print(self):
        ...
        return {
            'type': 'ir.actions.act_url',
            'url': '/report/html/reema_inventory.report_bladder_winding_balance/%s' % ','.join(str(i) for i in records.ids),
            'target': 'new',
        }
